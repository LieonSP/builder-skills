...
- Si la PRD demande qu'une ligne soit créée automatiquement suite à un événement sur `auth.users` (typiquement un profil créé à l'inscription), créer une fonction `SECURITY DEFINER` et un trigger `AFTER INSERT ON auth.users` qui l'appelle — cette exigence ne sera jamais satisfaite autrement, cette skill ne touchant jamais au frontend. Toujours fixer `SET search_path = ''` sur cette fonction et qualifier chaque table par son schéma à l'intérieur (`public.<table>`, jamais `<table>` seul) — sans ça, la fonction est vulnérable à un détournement de privilège par search_path (faille Postgres documentée, catégorie "Function Search Path Mutable" du Security Advisor Supabase). Ce n'est jamais optionnel, quelle que soit la fonction `SECURITY DEFINER` créée.
...
- **`search_path` toujours fixé sur une fonction `SECURITY DEFINER`.** Jamais d'exception, même pour une fonction simple — c'est une faille de sécurité documentée (Supabase Security Advisor), pas une préférence de style.
